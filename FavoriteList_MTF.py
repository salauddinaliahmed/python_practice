...
# Utilizing move to front heuristic.
class FavoriteListMTF(FavoriteList):
    # Overwritten methods.
    def _move_up(self,p):
        if p != self._data.first():
            self._data.add_first(self._data.delete(p)) # This step inserts and deletes as the delete function
                                                       # Returns a value/element.

    def top(self,k):
        if not 1 <= k <= len(self._data):
            raise ValueError("Please enter value within limits")
        temp = PositionalList()
        # Creating a new list, with values from the Favorite List.
        for each_item in self._data:
                temp.add_last(each_item)
        for i in range(k):
            high_pos = temp.first()
            walk = temp.after(high_pos)
            while walk is not None:
                # If the next element is greater than the first element, then we swap the high position with next position.
